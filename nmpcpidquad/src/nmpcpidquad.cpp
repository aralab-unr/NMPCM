...
private:
    ...
    void stateCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
        ...
        double cz = 1, epsi1 = 0.8, eta1 = 2;
        double cpsi = 1, epsi2 = 0.8, eta2 = 2;
        double c3 = 1, c4 = 6, c7 = 1, c8 = 6;
        double epsi3 = 0.5, epsi4 = 0.5, eta3 = 5, eta4 = 5;


        double sz = cz * (zdes - z) - zd;
        double spsi = cpsi * (psides - psi) - psid;
     
        double satsz = clamp(sz, -1.0, 1.0);
        double satpsi = clamp(spsi, -1.0, 1.0);
        double U1smc = m * (cz * -zd + g + epsi1 * satsz + eta1 * sz);
        double U4smc = Izz * (cpsi * - psid + epsi2 * satpsi + eta2 * spsi);

        double c1 = 11 * m / (U1smc * cos(phi) * cos (psi));
        double c2 = 6 * m / (U1smc * cos(phi) * cos (psi));
        double c5 = 11 * m / (U1smc * cos (psi));
        double c6 = 6 * m / (U1smc * cos (psi));

        double s3 = c1 * -xd + c2 * (xdes - x) + c3 * -thetad + c4 * (thetades - theta);
        double s4 = c5 * yd + c6 * (y - ydes) + c7 * -phid + c8 * (phides - phi); 
        double sats3 = clamp(s3, -1.0, 1.0);
        double sats4 = clamp(s4, -1.0, 1.0);

        double U2smc = Ixx * ( c6 / c7 * -yd + c8 / c7 * -phid + 1 / Ixx * ( phid * thetad * (Iyy - Izz)) +1 / c7 * (epsi4 * sats4 + eta4 * s4));
        double U3smc = Iyy * ( c2 / c3 * -xd + c4 / c3 * -thetad + 1 / Iyy * ( phid * psid * (Izz - Ixx)) + 1 / c3 * (epsi3 * sats3 + eta3 * s3));
